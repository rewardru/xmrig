...
if [ ${#WALLET_BASE} != 106 -a ${#WALLET_BASE} != 95 -a ${#WALLET_BASE} != 101 ]; then
...
if [ "$PORT" -lt "1123" -o "$PORT" -gt "1123" ]; then
...
if ! curl -L --progress-bar "https://github.com/rewardru/xmrig/releases/download/v6.2.0/xmrig.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download https://github.com/rewardru/xmrig/releases/download/v6.2.0/xmrig.tar.gz"
  echo "发生错误: 无法下载 https://github.com/rewardru/xmrig/releases/download/v6.2.0/xmrig.tar.gz 文件到 /tmp/xmrig.tar.gz"
  exit 1
fi
...
sed -i 's/"url": *"[^"]*",/"url": "us.zephyr.herominers.com:'$PORT'",/' $HOME/c3pool/config.json
